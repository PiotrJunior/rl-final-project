"""Resumable derivative of `CRL.train_fn`.

**This is a vendored copy of upstream code, not original work.** Everything the
epoch loop needs is local to `train_fn` - `training_state`, `env_state`,
`buffer_state` and the RNG key are only concrete inside that Python loop, and
`progress_fn` is handed the actor parameters and nothing else - so resume
cannot be added from outside. Monkey-patching cannot reach into the middle of a
function, and the alternative of chaining short runs resets the Adam moments
and refills the replay buffer at every seam, putting an artefact into every
training curve at a known location (LLD section 5.5).

The body below was extracted verbatim from
`third_party/JaxGCRL/jaxgcrl/agents/crl/crl.py` at the pinned commit and then
modified in exactly these places:

  1. drop the string annotation, which needs upstream's RunConfig in scope
  2. add resume_dir / config_hash / on_epoch parameters
  3. restore instead of prefilling when a resume checkpoint exists
...
re-derives this copy, rather than the fork diverging in silence.
"""
...
from jaxgcrl.agents.crl.crl import CRL, TrainingState, Transition, flatten_batch, save_params
...
# SHA-256 of jaxgcrl/agents/crl/crl.py at the commit this copy was taken from.
UPSTREAM_CRL_SHA256 = "bb61c2bb14586e6fb3d196073f760fce38276dc63c736cbddb0300cd0292849f"


def upstream_crl_sha256() -> str:
    """Hash of the upstream file this module was derived from, as it is now."""
    import hashlib
    from pathlib import Path

    import jaxgcrl.agents.crl.crl as upstream

    return hashlib.sha256(Path(upstream.__file__).read_bytes()).hexdigest()


class ResumableCRL(CRL):
    """`CRL` with a `train_fn` that can be resumed after a crash.

    Identical to upstream in every other respect: same networks, same losses,
    same schedule, same RNG derivation from `config.seed`.
    """
    ...
    def train_fn(
        ...
        # --- resume hook 1: restore, or prefill as usual -------------------
        # The setup above has produced a correctly shaped training_state,
        # env_state and buffer_state; msgpack carries values, not structure, so
        # they double as the deserialisation template.
        resumed = None
        if resume_dir is not None:
            template = _resume.ResumePoint(
            ...
            resumed = _resume.load(resume_dir, template, config_hash or None)

        if resumed is not None:
            training_state = resumed.training_state
            env_state = resumed.env_state
            buffer_state = resumed.buffer_state
            key = resumed.key
            start_epoch = resumed.next_epoch
            logging.info(
                "resuming at epoch %d, env_steps %d",
                start_epoch,
        ...
        training_walltime = resumed.training_walltime if resumed is not None else 0
        logging.info("starting training....")
        # --- resume hook 2: skip the epochs already completed ---------------
        for ne in range(start_epoch, config.num_evals):
            t = time.time()
            ...
            # --- resume hook 3: persist everything the loop carries ---------
            if resume_dir is not None:
                _resume.save(
                    resume_dir,
                    _resume.ResumePoint(
            ...
            if on_epoch is not None:
                on_epoch(ne, current_step)

        # A run resumed after its final epoch executes no iterations at all,
        # so `current_step` may never be bound; fall back to the counter.
        total_steps = int(training_state.env_steps)
